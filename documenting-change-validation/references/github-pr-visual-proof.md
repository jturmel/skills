...
Immediately after the completed `## Visual Proof` section, render the `## ⚠️ High-Risk Items for Human Review` block required by the main skill. The risk table's file links use immutable code blob permalinks; do not reuse screenshot asset URLs for code locations.
